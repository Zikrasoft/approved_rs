...
const byDateDesc = (
  a: CollectionEntry<'cases' | 'autoserviceCases' | 'detailingCases'>,
  b: CollectionEntry<'cases' | 'autoserviceCases' | 'detailingCases'>,
...
export const getPublishedAutoserviceCases = async () =>
  (await getCollection('autoserviceCases', (c) => c.data.published)).sort(
    byDateDesc,
  );

export const getPublishedDetailingCases = async () =>
  (await getCollection('detailingCases', (c) => c.data.published)).sort(
    byDateDesc,
  );
