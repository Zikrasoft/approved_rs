---
title: Mercedes-Benz C200
translations:
  en:
    title: 'Mercedes-Benz C-Class — service and maintenance in Belgrade'
    body: >
      ## Mercedes-Benz C-Class — service and maintenance in Belgrade


      Our auto service in Belgrade recently completed a full maintenance service on a **Mercedes-Benz C-Class W205**.


      As part of the **Mercedes-Benz minor service**, we carried out:


      - engine oil replacement;

      - oil filter replacement;

      - engine air filter replacement;

      - cabin air filter replacement;

      - replacement of all brake pads;

      - inspection and servicing of the complete braking system.


      We paid special attention to the braking system. **Brake pads are one of the most important safety components of any vehicle**, so regular inspection and timely replacement are essential. During brake service, it is important to inspect not only the pads but also the brake discs, calipers and guides to ensure proper and even braking.


      Regular Mercedes-Benz maintenance helps prevent more expensive repairs, keeps the vehicle reliable and ensures safe everyday driving.


      **Mercedes-Benz service in Belgrade**\

      Diagnostics, routine maintenance, oil and filter replacement, brake service and Mercedes-Benz repairs.

  sr:
    title: 'Mercedes-Benz C-Class — servis i održavanje u Beogradu'
    body: >
      ## Mercedes-Benz C-Class — servis i održavanje u Beogradu


      U našem auto-servisu u Beogradu obavili smo kompletan servis na **Mercedes-Benz C-Class W205**.


      U okviru **malog servisa Mercedes-Benz** urađeno je:


      - zamena motornog ulja;

      - zamena filtera ulja;

      - zamena filtera vazduha motora;

      - zamena filtera kabine;

      - zamena svih kočionih pločica;

      - kompletna kontrola i servis kočionog sistema.


      Posebnu pažnju posvetili smo kočnicama. **Kočioni sistem je jedan od najvažnijih sistema za bezbednost vozila**, zato je njegova redovna kontrola veoma važna. Prilikom zamene pločica potrebno je proveriti i stanje diskova, čeljusti i vođica kako bi kočenje bilo pravilno i ravnomerno.


      Redovno održavanje Mercedes-Benz vozila pomaže da se spreče ozbiljniji kvarovi, produži vek trajanja komponenti i obezbedi sigurna vožnja.


      **Mercedes-Benz servis u Beogradu**\

      Dijagnostika, mali servis, zamena ulja i filtera, servis kočnica i popravka Mercedes-Benz vozila.
  es:
    title: Mercedes-Benz C200
    body: |-
      ## Mercedes-Benz C-Class — Mantenimiento en nuestro servicio en Belgrado

      En nuestro taller en Belgrado, realizamos un servicio de mantenimiento integral para el **Mercedes-Benz C-Class W205**.

      Dentro de los **servicios de mantenimiento menor de Mercedes-Benz**, llevamos a cabo:

      - Cambio de aceite del motor;
      - Sustitución del filtro de aceite;
      - Reemplazo del filtro de aire del motor;
      - Cambio del filtro del habitáculo;
      - Sustitución de todas las pastillas de freno;
      - Mantenimiento y revisión del sistema de frenos.

      Se prestó especial atención a los frenos. **Las pastillas de freno son uno de los elementos clave para la seguridad del vehículo**, por lo que su estado debe ser revisado regularmente. Al reemplazarlas, es crucial verificar no solo las pastillas, sino también el estado de los discos de freno, los calipers y los soportes, para asegurar que el sistema de frenos funcione correctamente y de manera uniforme.

      El mantenimiento regular ayuda a preservar la fiabilidad de Mercedes-Benz, evita fallos más graves y asegura una conducción segura del vehículo.

      **Mantenimiento de Mercedes-Benz en Belgrado**\ Diagnóstico, mantenimiento, cambio de aceite y filtros, servicio del sistema de frenos y reparación de vehículos Mercedes-Benz.
  de:
    title: Mercedes-Benz C200
    body: |-
      ## Mercedes-Benz C-Klasse — Service in unserer Werkstatt in Belgrad

      In unserer Werkstatt in Belgrad haben wir eine umfassende technische Wartung für den **Mercedes-Benz C-Klasse W205** durchgeführt.

      Im Rahmen des **kleinen Services für Mercedes-Benz** wurden folgende Arbeiten durchgeführt:

      - Motorölwechsel;
      - Austausch des Ölfilters;
      - Austausch des Luftfilters für den Motor;
      - Austausch des Innenraumfilters;
      - Austausch aller Bremsbeläge;
      - Wartung und Überprüfung des Bremssystems.

      Besonderes Augenmerk wurde auf die Bremsen gelegt. **Die Bremsbeläge sind eines der wichtigsten Sicherheitselemente des Fahrzeugs**, daher ist es notwendig, ihren Zustand regelmäßig zu überprüfen. Bei einem Austausch ist es wichtig, nicht nur die Beläge, sondern auch den Zustand der Bremsscheiben, der Sättel und der Führungen zu kontrollieren, damit das Bremssystem korrekt und gleichmäßig funktioniert.

      Regelmäßige technische Wartung hilft, die Zuverlässigkeit des Mercedes-Benz zu erhalten, schwerwiegende Störungen zu vermeiden und eine sichere Nutzung des Fahrzeugs zu gewährleisten.

      **Service für Mercedes-Benz in Belgrad**\
      Diagnose, Wartung, Öl- und Filterwechsel, Wartung des Bremssystems und Reparatur von Mercedes-Benz-Fahrzeugen.

car: Mercedes-Benz C-Class
year: 2018
servicesApplied:
  - diagnostics
  - maintenance
  - suspension
image: image.jpg
gallery: []
date: 2026-05-05
published: true
translatedFrom: 62404b84407bec57
---
## Mercedes-Benz C-Class — обслуживание в нашем сервисе в Белграде

В нашем автосервисе в Белграде провели комплексное техническое обслуживание **Mercedes-Benz C-Class W205**.

В рамках **малого ТО Mercedes-Benz** выполнили:

- замену моторного масла;
- замену масляного фильтра;
- замену воздушного фильтра двигателя;
- замену салонного фильтра;
- замену всех тормозных колодок;
- обслуживание и проверку тормозной системы.

Отдельное внимание уделили тормозам. **Тормозные колодки — один из ключевых элементов безопасности автомобиля**, поэтому их состояние необходимо регулярно контролировать. При замене важно проверить не только сами колодки, но и состояние тормозных дисков, суппортов и направляющих, чтобы тормозная система работала корректно и равномерно.

Регулярное техническое обслуживание помогает сохранить надежность Mercedes-Benz, избежать более серьезных неисправностей и обеспечить безопасную эксплуатацию автомобиля.

**Обслуживание Mercedes-Benz в Белграде**\
Диагностика, ТО, замена масла и фильтров, обслуживание тормозной системы и ремонт автомобилей Mercedes-Benz.
