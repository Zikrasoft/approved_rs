...
export function generateMeta(
  ...
  switch (service) {
    case 'autopodbor': {
      const location = city
        ? `в ${city.ru.nameLocative}`
        : `в ${country!.ru.nameLocative}`;
      return {
    ...
    case 'vykup': {
      return {
        title: `Выкуп авто в ${country!.ru.nameLocative} на иностранных номерах — срочно | ${SITE_NAME}`,
        description: `Срочный выкуп автомобилей в ${country!.ru.nameLocative} на любых номерах — иностранных, российских или сербских. Оценка онлайн, оформление за 1 день, перевод в день сделки.`,
        canonical,
        ogImage,
      };
    }
    case 'proverka': {
      return {
        title: `Проверка авто в ${country!.ru.nameLocative} перед покупкой | ${SITE_NAME}`,
        description: `Независимая проверка автомобиля в ${country!.ru.nameLocative}. Выезд эксперта, осмотр кузова и ходовой, отчёт. Защитите себя от покупки битого авто.`,
        canonical,
        ogImage,
      };
    }
