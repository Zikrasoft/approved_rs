...
vi.mock('node:fs/promises', () => ({
  ...
  readFile: vi.fn().mockResolvedValue('title: BMW\nimage: ./image.jpg\ndate: 2026-01-01\n'),
...
function makeCtx(opts: { cookie?: string; dir?: string; slug?: string; photos?: string[] } = {}): APIContext {
  ...
  const request = new Request('http://localhost/api/admin/case-photos-upload', { method: 'POST', body: form });
  return {
    request,
    cookies: { get: (name: string) => (opts.cookie && name === 'keystatic-gh-access-token' ? { value: opts.cookie } : undefined) },
...
describe('POST /api/admin/case-photos-upload', () => {
  ...
  describe('prod (cookie required)', () => {
    beforeEach(() => {
      ...
      vi.mocked(getGithubFile).mockResolvedValue('title: BMW\nimage: ./image.jpg\ndate: 2026-01-01\n');
    ...
    it('commits via the GitHub API and redirects with ok=N when the cookie is present', async () => {
      const res = await POST(makeCtx({ cookie: 'gh-token', photos: ['a.jpg'] }));
      expect(res.status).toBe(303);
      expect(res.headers.get('location')).toContain('ok=1');
      expect(commitGalleryPhotos).toHaveBeenCalledWith('gh-token', expect.objectContaining({
        indexPath: 'src/content/cases/bmw-320/index.md',
      }));
    });

    it('reports 404 when the case file is missing on GitHub', async () => {
      vi.mocked(getGithubFile).mockRejectedValueOnce(new GithubApiError(404, 'x', 'not found'));
      const res = await POST(makeCtx({ cookie: 'gh-token' }));
      expect(res.status).toBe(404);
    });

    it('propagates a non-404 GitHub failure instead of reporting "not found"', async () => {
      vi.mocked(getGithubFile).mockRejectedValueOnce(new GithubApiError(500, 'x', 'boom'));
  ...
  it('rejects a file over the size cap with 413, regardless of environment', async () => {
    ...
    form.append('photos', new File([new Uint8Array(11 * 1024 * 1024)], 'huge.jpg', { type: 'image/jpeg' }));
    const ctx = {
      request: new Request('http://localhost/api/admin/case-photos-upload', { method: 'POST', body: form }),
