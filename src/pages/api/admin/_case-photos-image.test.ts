...
function makeCtx(opts: { dir?: string; slug?: string; file?: string; cookie?: string } = {}): APIContext {
...
describe('GET /api/admin/case-photos-image', () => {
  ...
  it('dev: reads the file off disk and returns it with the right content type', async () => {
    ...
    expect(readFile).toHaveBeenCalledWith(expect.stringContaining('src/content/cases/bmw-320/gallery/photo.jpg'));
  });
  ...
  it('prod: fetches the file from GitHub when the cookie is present', async () => {
    import.meta.env.PROD = true;
    vi.mocked(getGithubFileBuffer).mockResolvedValue(Buffer.from('fake-image-bytes'));
    const res = await GET(makeCtx({ cookie: 'gh-token' }));
    expect(res.status).toBe(200);
    expect(getGithubFileBuffer).toHaveBeenCalledWith('gh-token', 'src/content/cases/bmw-320/gallery/photo.jpg');
  });

  it('prod: 404s when the file is missing on GitHub, instead of a raw 500', async () => {
    import.meta.env.PROD = true;
    vi.mocked(getGithubFileBuffer).mockRejectedValueOnce(new GithubApiError(404, 'x', 'not found'));
    const res = await GET(makeCtx({ cookie: 'gh-token' }));
  ...
  it('prod: propagates a non-404 GitHub failure', async () => {
    import.meta.env.PROD = true;
    vi.mocked(getGithubFileBuffer).mockRejectedValueOnce(new GithubApiError(500, 'x', 'boom'));
