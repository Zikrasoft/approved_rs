...
export const BOT_TOKEN = requireEnv('TELEGRAM_BOT_TOKEN', import.meta.env.TELEGRAM_BOT_TOKEN);
export const GROUP_ID = requireEnv('TELEGRAM_GROUP_ID', import.meta.env.TELEGRAM_GROUP_ID);
export const BOT_USERNAME = requireEnv('TELEGRAM_BOT_USERNAME', import.meta.env.TELEGRAM_BOT_USERNAME);
export const OWNER_ID = import.meta.env.TELEGRAM_OWNER_ID;
export const ADMIN_ID = import.meta.env.TELEGRAM_ADMIN_ID;
...
export function expectMessageId(sent: unknown, context: string): number {
  ...
  if (typeof messageId !== 'number') throw new Error(`[telegram] ${context} response missing message_id`);
  return messageId;
}

export function expectMessageAndChatId(sent: unknown, context: string): { messageId: number; chatId: number } {
  const result = sent as { message_id?: unknown; chat?: { id?: unknown } } | null;
  const chatId = result?.chat?.id;
  if (typeof chatId !== 'number') throw new Error(`[telegram] ${context} response missing chat.id`);
  return { messageId: expectMessageId(sent, context), chatId };
}

// A double-tap's "message is not modified" isn't a real failure.
export async function safeEditMessage(chatId: number, messageId: number, text: string, keyboard: object): Promise<void> {
  try {
...
export async function sendForceReplyPrompt(chatId: number, text: string): Promise<number> {
  const sent = await tgPost('sendMessage', { chat_id: chatId, text, reply_markup: { force_reply: true, selective: true } });
