...
function formatLeadText(lead: StoredLead, role: Role): string {
  const channelLabel = isTrackedContactChannel(lead.contactChannel)
    ? CONTACT_CHANNEL_LABELS[lead.contactChannel]
    : undefined;
...
export function deepLinkKeyboard(id: number): Keyboard {
  return {
    inline_keyboard: [[{ text: '📂 Открыть в боте', url: deepLink(id) }]],
  };
}
...
export function buildStats(leads: StoredLead[], role: Role): string {
  ...
  const totalEarned = roundMoney(
    wonLeads.reduce((sum, l) => sum + l.dealAmount, 0),
  );
  const commissionTotal = roundMoney(
    wonLeads.reduce((sum, l) => sum + getCommission(l).commission, 0),
  );
  const paidTotal = roundMoney(
    wonLeads.reduce((sum, l) => sum + l.paidAmount, 0),
  );
  const remainingTotal = roundMoney(
    wonLeads.reduce((sum, l) => sum + getCommission(l).remaining, 0),
  );
