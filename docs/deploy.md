...
### Аналитика (необязательные)

| Переменная                 | approved.rs | carlab.rs | details.rs | Фолбэк в коде       | Без неё             |
| -------------------------- | ----------- | --------- | ---------- | ------------------- | ------------------- |
| `PUBLIC_YM_COUNTER_ID`     | —           | ✅        | ✅         | — / нет (выключена) | Метрика не грузится |
| `PUBLIC_GA_MEASUREMENT_ID` | —           | ✅        | ✅         | — / нет (выключена) | GA не грузится      |

Счётчики approved.rs зашиты в `src/utils/constants.ts`, переменные он не
читает. У двух брендовых сайтов своих счётчиков пока нет: без переменной
`defineAnalytics` не грузит ничего — ни одного запроса к `mc.yandex.ru` или
`googletagmanager.com`. Это не деградация, а выключенное состояние; счётчик
заводится отдельно на каждый домен.

Значение парсится через `Number()`, поэтому опечатка в цифрах (буква `O`
вместо нуля) даёт `NaN` — тихо выключенную аналитику без единой ошибки в
сборке. После установки счётчика стоит проверить, что запрос к `tag.js`
действительно уходит.

**Включить аналитику на брендовом сайте — это не только переменная.** Их
страницы политики сейчас прямо утверждают, что аналитических кук нет, а баннер
согласия говорит только про анонимный идентификатор для заявок. Вместе с
переменной надо переписать русские строки `privacy` в
`src/content/i18n/pages.yaml` и `cookie.notice` в `site.yaml`, а потом поднять
`COOKIE_POLICY_VERSION` в `src/utils/constants.ts` — иначе у всех, кто уже
ответил на баннер, согласие молча начнёт означать другое.

У approved.rs политика описывает текущее поведение: аналитика стартует на
первой загрузке, отказ действует со следующей, Метрика пишет сессии, а форма
заявки из записи исключена (`ym-hide-content ym-disable-keys` на
`LeadForm.astro`).
