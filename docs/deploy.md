...
- **pnpm v11 + Turborepo** — монорепо, приложения в `apps/*`, общие пакеты в
  `packages/*`. Сборка и деплой идут в GitHub Actions (см. «Настроить деплой
  из GitHub Actions» ниже)
- **Vercel** — хостинг, Edge Middleware для geo-таргетинга

> Монорепо: сайт approved.rs живёт в `apps/approved-rs/`. Все пути в этом
> документе — относительно него (`vercel.json` = `apps/approved-rs/vercel.json`),
> кроме `.github/` и конфигов линтеров, которые лежат в корне репозитория.
> Vercel CLI определяет проект по рабочей директории, поэтому джоба `deploy`
> целиком выполняется из `apps/approved-rs`.
...
(lint/prettier/typecheck/test) → джоба `translate` → джоба `deploy` (`vercel
...
`translate` идёт до деплоя, а не параллельно ему: скрипты перевода отрабатывают,
коммит с переводами уходит в ту же ветку, и `deploy` чекаутит именно тот SHA,
который остался после джобы (`needs.translate.outputs.sha`). Если переводить
было нечего, SHA — исходный коммит пуша. Так в прод физически не может уехать
контент без переводов, и деплой на пуш ровно один.
...
(**в любую ветку**, не только `main`) она прогоняет скрипты перевода каждого
приложения и коммитит результат обратно — так переводы уже есть в фиче-ветке
к моменту мержа, а не появляются только после него.
...
принципе не может запустить лишний билд. На `main` джоба `deploy` идёт следом
за `translate` в том же прогоне и чекаутит именно тот коммит, который
`translate` оставил после себя, — так что в прод никогда не уезжает контент
без переводов.
...
следующего успешного запуска (или ручного прогона из `apps/approved-rs`:
`node --env-file=.env --experimental-strip-types scripts/translate-cases.ts`).
Оба скрипта ищут контент относительно текущей директории, поэтому из корня
монорепо они не найдут ничего — запускать только изнутри приложения.
