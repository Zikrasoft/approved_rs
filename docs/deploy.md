...
Вместо неё — `.github/workflows/translate-cases.yml`: при каждом пуше в
`main`, который трогает файл кейса, джоба прогоняет `scripts/translate-cases.ts`
и коммитит результат обратно.
